...
export function DesignSystemSections({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="design-system-sections">
      <section className="design-system-section" aria-labelledby="design-system-colors">
        ...
        <div className="design-system-color-grid">
          {colors.map(([token, className], index) => (
            <article className="design-system-color-card" key={token}>
              <div className={`design-system-color-swatch design-system-color-${className}`} aria-hidden="true" />
      ...
      <section className="design-system-section" aria-labelledby="design-system-typography">
        ...
        <div className="design-system-type-list">
          {copy.typeSamples.map((sample, index) => (
            <div className="design-system-type-specimen" key={samples[index]}>
              <p className="design-system-specimen-label">{copy.typeLabels[index]}</p>
      ...
      <section className="design-system-section" aria-labelledby="design-system-buttons">
        <h2 id="design-system-buttons">{copy.buttons}</h2>
        <div className="design-system-button-list">
          {copy.buttonLabels.map((label, index) => <span className={`design-system-button design-system-button-${index}`} key={label}>{label}</span>)}
        </div>
      </section>

      <section className="design-system-section" aria-labelledby="design-system-spacing">
        ...
        <div className="design-system-spacing-list">
          {copy.spacingLabels.map((label, index) => (
            <div className="design-system-spacing-card" key={label}>
              <span className={`design-system-spacing-bar design-system-spacing-bar-${spacing[index]}`} aria-hidden="true" />
              <span>{label}</span>
            </div>
          ))}
        </div>
