...
export function MethodsGroupsSection({ locale }: { locale: Locale }) {
  return (
    <section className="methods-groups" aria-label={locale === "fr" ? "Groupes de méthodes" : "Method groups"}>
      {methodGroups.map((group) => (
        <section className="methods-group" key={group.id}>
          <div className="methods-group-heading">
            <SectionHeading kicker={group.id} title={localize(group.title, locale)} />
          </div>
