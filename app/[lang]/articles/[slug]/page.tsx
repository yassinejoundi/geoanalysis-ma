...
export default async function ArticlePage({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  ...
  const entry = articles.find((item) => item.slug === slug);
  if (!entry) notFound();
  const relatedEntries = articles.filter((item) => item.id !== entry.id).slice(0, 3);

  return <EditorialDetail entry={entry} relatedEntries={relatedEntries} locale={lang} kind="articles" copy={articlePageCopy[lang]} />;
}
